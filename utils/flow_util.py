...
def visualize_flow(img, flows, path, mode='forward'):
    '''
    flows: shape [1, frames, 2, width, height]
    if you are using propainter, pass in gt_flows_bi[0 or 1], pred_flows_bi[0 or 1] for forward/backward flow
    result visualize in hsv (https://answers.opencv.org/upfiles/15186766673210035.png)
    e.g. red means moving right
    '''
    l = flows.shape[1]
    flow = flows[0]
    print("out path ", path)
    ...
    for i in range(l):
        ...
        bi_flow = flow[i].cpu().permute(1, 2, 0).numpy().astype(np.float32)
        
        mag, ang = cv2.cartToPolar(bi_flow[..., 0], bi_flow[..., 1])
        hsv[..., 0] = ang * 180 / np.pi / 2
        hsv[..., 2] = cv2.normalize(mag, None, 0, 255, cv2.NORM_MINMAX)
        bgr = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
        if not os.path.isdir(path):
            os.mkdir(path)
        if not os.path.isdir(f'{path}/forward'):
            os.mkdir(f'{path}/forward')
        if not os.path.isdir(f'{path}/backward'):
            os.mkdir(f'{path}/backward')
        if mode == 'backward':
            cv2.imwrite(f'{path}/backward/{i + 1}_to_{i}.jpg', bgr)
        else:
            cv2.imwrite(f'{path}/forward/{i}_to_{i + 1}.jpg', bgr)